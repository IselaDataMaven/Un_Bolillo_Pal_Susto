class Level3Scene extends Phaser.Scene {
  ...
  create() {
    ...
    // --- BACKGROUND: Vertical tunnel (single images, no tiling) ---
    // Place background images sequentially down the tunnel
    var bgScale = w / 800; // fit width
    for (var bgI = 0; bgI < 9; bgI++) {
      var bgKey = (bgI % 2 === 0) ? 'hoyo-bg-far' : 'hoyo-bg-mid';
      var bgImg = this.add.image(w / 2, bgI * 900 + 450, bgKey);
      bgImg.setScale(Math.max(w / bgImg.width, 900 / bgImg.height));
      bgImg.setDepth(-3);
    }

    // Organic wall edges (left and right)
    this.wallLeft = this.add.rectangle(15, worldHeight / 2, 30, worldHeight, 0x1a0f05)
      .setScrollFactor(1).setDepth(0).setAlpha(0.7);
    this.wallRight = this.add.rectangle(worldWidth - 15, worldHeight / 2, 30, worldHeight, 0x1a0f05)
      .setScrollFactor(1).setDepth(0).setAlpha(0.7);
    ...
    this.player = this.physics.add.sprite(worldWidth / 2, 80, 'hoyo-caida1');
    ...
    // Falling animation
    if (!this.anims.exists('chicles-fall')) {
      this.anims.create({
        key: 'chicles-fall',
        frames: [
          { key: 'hoyo-caida1' },
    ...
    this.player.anims.play('chicles-fall', true);
  ...
  completeLevel() {
    ...
    this.time.delayedCall(3000, () => {
      this.scene.start('Level4Scene', { score: this.score + this.killCount * 10 });
    });
  }
}
